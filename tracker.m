function [positions, runtime] = tracker(video_path, img_files, pos, target_sz, ...
...
%     format [rows, columns]).
%    RUNTIME is the tracker execution time, without video loading/rendering.
%
%   Joao F. Henriques, 2014


    [isResized, pos, target_sz] = resizeImage(pos, target_sz);
...
% 	%performance. in practice it is slower, due to the larger window size.
	...
	for frame = 1:numel(img_files),
		...
		%save position and timing for output
		positions(frame,:) = pos;
		runtime = runtime + toc();
...
% 			pause(0.05)  %uncomment to run slower
	...
	end % for frame = 1:numel(img_files),
...
function [isResized, pos, target_sz] = resizeImage(pos, target_sz)
%if the target is large, lower the resolution, we don't need that much
%detail
	isResized = (sqrt(prod(target_sz)) >= 100);  %diagonal size >= threshold
    if isResized
        pos = floor(pos / 2);
